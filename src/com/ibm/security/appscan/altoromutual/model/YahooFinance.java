...
public class YahooFinance {
    ...
    /**
    * Sends a basic quotes request to Yahoo Finance. This will return a {@link yahoofinance.Stock} object
    ...
    * @return           a {@link yahoofinance.Stock} object containing the requested information
     * @throws IOException when there's a connection problem
    */
    public static yahoofinance.Stock get(String symbol) throws IOException {
        return YahooFinance.get(symbol, false);
    }
    
    /**
     * Same as the <code>get(String)</code> method, but with the option to include
     * historical stock quote data. Including historical data will cause the {@link yahoofinance.Stock}
     ...
     * @throws IOException when there's a connection problem
     */
    public static yahoofinance.Stock get(String symbol, boolean includeHistorical) throws IOException {
        Map<String, yahoofinance.Stock> result = YahooFinance.getQuotes(symbol, includeHistorical);
     ...
     * @throws IOException when there's a connection problem
     */
    public static yahoofinance.Stock get(String symbol, Interval interval) throws IOException {
     ...
     * @throws IOException when there's a connection problem
     */
    public static yahoofinance.Stock get(String symbol, Calendar from) throws IOException {
     ...
     * @throws IOException when there's a connection problem
     */
    public static yahoofinance.Stock get(String symbol, Calendar from, Interval interval) throws IOException {
     ...
     * @throws IOException when there's a connection problem
     */
    public static yahoofinance.Stock get(String symbol, Calendar from, Calendar to) throws IOException {
     ...
     * @throws IOException when there's a connection problem
     */
    public static yahoofinance.Stock get(String symbol, Calendar from, Calendar to, Interval interval) throws IOException {
        Map<String, yahoofinance.Stock> result = YahooFinance.getQuotes(symbol, from, to, interval);
        return result.get(symbol.toUpperCase());
    }
    ...
    * that links the symbols to their respective {@link yahoofinance.Stock} objects.
    ...
    * @return           a Map that links the symbols to their respective Stock objects
     * @throws IOException when there's a connection problem
    */
    public static Map<String, yahoofinance.Stock> get(String[] symbols) throws IOException {
     ...
     * @throws IOException when there's a connection problem
     */
    public static Map<String, yahoofinance.Stock> get(String[] symbols, boolean includeHistorical) throws IOException {
        return YahooFinance.getQuotes(yahoofinance.Utils.join(symbols, ","), includeHistorical);
    }
     ...
     * @throws IOException when there's a connection problem
     */
    public static Map<String, yahoofinance.Stock> get(String[] symbols, Interval interval) throws IOException {
        return YahooFinance.getQuotes(yahoofinance.Utils.join(symbols, ","), HistQuotesRequest.DEFAULT_FROM, HistQuotesRequest.DEFAULT_TO, interval);
    }
     ...
     * @throws IOException when there's a connection problem
     */
    public static Map<String, yahoofinance.Stock> get(String[] symbols, Calendar from) throws IOException {
        return YahooFinance.getQuotes(yahoofinance.Utils.join(symbols, ","), from, HistQuotesRequest.DEFAULT_TO, HistQuotesRequest.DEFAULT_INTERVAL);
    }
     ...
     * @throws IOException when there's a connection problem
     */
    public static Map<String, yahoofinance.Stock> get(String[] symbols, Calendar from, Interval interval) throws IOException {
        return YahooFinance.getQuotes(yahoofinance.Utils.join(symbols, ","), from, HistQuotesRequest.DEFAULT_TO, interval);
    }
     ...
     * @throws IOException when there's a connection problem
     */
    public static Map<String, yahoofinance.Stock> get(String[] symbols, Calendar from, Calendar to) throws IOException {
        return YahooFinance.getQuotes(yahoofinance.Utils.join(symbols, ","), from, to, HistQuotesRequest.DEFAULT_INTERVAL);
    }
     ...
     * @throws IOException when there's a connection problem
     */
    public static Map<String, yahoofinance.Stock> get(String[] symbols, Calendar from, Calendar to, Interval interval) throws IOException {
        return YahooFinance.getQuotes(yahoofinance.Utils.join(symbols, ","), from, to, interval);
    }
    ...
    public static Map<String, FxQuote> getFx(String[] symbols) throws IOException {
        List<FxQuote> quotes;
        if(YahooFinance.QUOTES_QUERY1V7_ENABLED.equalsIgnoreCase("true")) {
            FxQuotesQuery1V7Request request = new FxQuotesQuery1V7Request(yahoofinance.Utils.join(symbols, ","));
    ...
    private static Map<String, yahoofinance.Stock> getQuotes(String query, boolean includeHistorical) throws IOException {
        Map<String, yahoofinance.Stock> result = new HashMap<String, yahoofinance.Stock>();
        if(YahooFinance.QUOTES_QUERY1V7_ENABLED.equalsIgnoreCase("true")) {
            StockQuotesQuery1V7Request request = new StockQuotesQuery1V7Request(query);
            List<yahoofinance.Stock> stocks = request.getResult();
            for(yahoofinance.Stock stock : stocks) {
            ...
            for(StockQuotesData data : quotes) {
                yahoofinance.Stock s = data.getStock();
                result.put(s.getSymbol(), s);
            }
        }

        if(includeHistorical) {
            for(yahoofinance.Stock s : result.values()) {
                s.getHistory();
            }
        }
    ...
    private static Map<String, yahoofinance.Stock> getQuotes(String query, Calendar from, Calendar to, Interval interval) throws IOException {
        Map<String, yahoofinance.Stock> stocks = YahooFinance.getQuotes(query, false);
        stocks = YahooFinance.fetchHistoricalQuotes(stocks, from, to, interval);
        return stocks;
    }
    
    private static Map<String, yahoofinance.Stock> fetchHistoricalQuotes(Map<String, yahoofinance.Stock> stocks, Calendar from, Calendar to, Interval interval) throws IOException {
